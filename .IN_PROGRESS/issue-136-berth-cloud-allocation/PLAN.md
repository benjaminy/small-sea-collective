...
- Replace `_bucket_name_for_protocol` at the two Core-bootstrap sites (`provisioning.py:4132` and `provisioning.py:4371`) with reads from the auto-allocated Core `berth_cloud_allocation.location` so invitation descriptors and `team_device` writes stay consistent with allocation during the legacy-fallback period.
...
- Hub materialization:
  ...
  - The own-storage flow is: **resolve allocation → build adapter → adapter.materialize() → adapter.upload/download**. The backend orchestrates; the adapter implements.
...
- Replace `provisioning.py:4132` (`creator_bucket = _bucket_name_for_protocol(...)`) with a read from the freshly-allocated Core location, falling back to `None` when no allocation was created. The `team_device.bucket` write at line 4145 then reflects the allocation rather than the formula, so the legacy fallback path stays consistent through the transition.
...
- Replace `provisioning.py:4371` (`inviter_bucket = _bucket_name_for_protocol(...)`) with a read from the inviter's Core allocation. This is the field that the invitee uses to bootstrap the team repo, so a mismatch here breaks the very first invitation.
...
6. No production code path depends on the legacy `_bucket_name_for_protocol` for own-storage routing (it remains for `team_device` writes — Slice C).
...
6. Peer-read paths and `team_device` are untouched in this branch.
