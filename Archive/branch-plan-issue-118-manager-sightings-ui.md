...
- `refresh_app_sightings()` becomes a current-prompt API, not a raw Hub-observation passthrough. Re-run the existing sightings micro tests early; if any assertion assumes the stored Hub `reason` is returned verbatim after local state changes, update the test to assert the current reason instead of weakening the new behavior.
...
- **Post-action refresh can shadow success.** Register, Activate, and Dismiss mutate local state without the Hub, but each route refreshes through the Hub afterward. If the NoteToSelf session expires between the user's Refresh and action click, the action may commit locally while the post-action refresh renders an inline error. Acceptable for v1 as long as the error copy says to reconnect and Refresh; after reconnecting, the current-state recheck should show the committed result.
