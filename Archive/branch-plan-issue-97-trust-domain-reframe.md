...
### Fifth revision (this pass)

Three targeted refinements closing the three remaining committee concerns:

- **Member/device bridge stated as a derivation, not an assertion.** The decision section now names the concrete crypto chain that bridges governance (member-scoped admin votes) and crypto (device key signatures): `device_link` certs at the anchor state map device public keys to `member_id`s; cert validity is grounded by write-acceptance cert-chain rules (point 5); an approval is valid iff its signing key appears in a `device_link` cert at the anchor that maps to a current-admin `member_id`. This closes the seam by making the bridge a step-by-step derivation that a verifier can replay independently.
- **Transcript immutability reasoning made explicit.** The decision section now explains *why* device keys can be safely bound (the invitee generates them fresh at acceptance time; they are immutable from the moment of signing) and why transport state must be excluded (may not yet exist at signing time or may legitimately change post-admission; freezing it would force either pre-admission cloud provisioning or an aging signed artifact). The conclusion was already correct; the rationale is now present for independent verification.
- **Proposal shell visibility tightened: "before the invitee is contacted," not just "before the invitee has responded."** The ordering language in the decision section now unambiguously places other admins' first view of the proposal before the invitee is even aware of the invitation. The previous wording ("before the invitee has responded") admitted the reading that admins only see the shell after the invitee has already engaged; this revision removes that ambiguity.
