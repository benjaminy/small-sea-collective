# Design Record: Files Materialization Coordinates

Files has an explicit `FilesMaterializationContext` carrying
`(participant_hex, team_id, team_name, app_name)`. The Hub session is the one
and only source of `team_id`: every Files context is constructed from a
...
A Files operation needs to contact the Hub in exactly two situations:

1. **First-time team discovery.** The first interaction with a friendly
   `team_name` requires a session so Files can learn the corresponding
...
All other Files operations resolve `team_name → team_id` offline by reading
...
There is no legacy fallback. Files operations either have a real context
derived from a real session, or they error.

## Local layout

```text
{files_root}/participants/{participant_hex}/teams/{team_id}/
...
boundary. Files uses `registry/` and `niches/{niche_name}/` only; it does
...
- `files.*` operations take a `FilesMaterializationContext`. The
...
- `sync.resolve_team_context(files_root, participant_hex, team_name)`
...
- `sync.login_team(files_root, ...)` is the CLI entry point that
  legitimately needs the Hub to discover `team_id`.
  It delegates to `sync.finalize_login(files_root, team_name, participant_hex, session)`,
  the shared helper that validates `session_info`,
  calls `files.materialize_team`,
...
- `files.iter_materialized_teams` validates each `metadata.json` against
  Files' integrity rules: `app_name` must equal `"SmallSeaCollectiveFiles"`,
...
- Team creation is a Manager function and does not go through Files.
  ...
  `ssc-files login <team_name>` on the CLI.
