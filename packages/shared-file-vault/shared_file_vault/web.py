...
async def niche_detail(request: Request, team_name: str, niche_name: str):
    ctx = _ctx(request)
    try:
        entries = vault.status(
            ctx["vault_root"], ctx["participant_hex"], team_name, niche_name
        )
    except ValueError:
        entries = None
    try:
...
@app.post(
    "/teams/{team_name}/niches/{niche_name}/checkout", response_class=HTMLResponse
)
async def checkout_niche(
    request: Request, team_name: str, niche_name: str, dest_path: str = Form(...)
):
    ctx = _ctx(request)
    vault.checkout_niche(
        ctx["vault_root"], ctx["participant_hex"], team_name, niche_name, dest_path
    )
    entries = vault.status(
        ctx["vault_root"], ctx["participant_hex"], team_name, niche_name
    )
...
async def publish_niche(
    request: Request, team_name: str, niche_name: str, message: str = Form(None)
):
    ctx = _ctx(request)
