...
def test_team_manager_connect(playground_dir):
    """TeamManager.connect() should open a Hub session via the two-step PIN flow."""
    import pathlib
    import small_sea_hub.backend as SmallSea
    import small_sea_manager.provisioning as Provisioning
    from fastapi.testclient import TestClient
    ...
    manager = TeamManager(root, participant_hex, _http_client=http)

    # Capture the PIN that the backend generates so the pin_provider can return it.
    captured = {}
    ...
    try:
        manager.connect(pin_provider=lambda _: captured["pin"])
    finally:
        backend.request_session = _orig

    assert manager.session is not None
    assert manager.session.token  # non-empty token hex


def test_team_manager_connect_requires_pin_provider(playground_dir):
    """connect() without a pin_provider raises a clear error."""
    import pathlib
    import small_sea_hub.backend as SmallSea
    import small_sea_manager.provisioning as Provisioning
    from fastapi.testclient import TestClient
    ...
    with pytest.raises(RuntimeError, match="pin_provider"):
        manager.connect()
