"""Core operations for Small Sea Files.

See spec.md for the design. Key points:

- One Files root per device/participant.
  files_root + participant_hex identify it.
...
class FilesMaterializationContext:
    ...
    def from_session_info(cls, info):
        ...
        if missing:
            raise ValueError(
                "Hub session_info missing required Files materialization field(s): "
                + ", ".join(missing)
            )
        if info.get("app_name") != "SmallSeaCollectiveFiles":
            raise ValueError(
                "Hub session_info app_name must be 'SmallSeaCollectiveFiles' for Files "
...
def _validate_context(participant_hex, context):
    if not isinstance(context, FilesMaterializationContext):
        raise TypeError(
            "Files operations require a FilesMaterializationContext; "
            f"got {type(context).__name__}"
        )
    if context.participant_hex != participant_hex:
        raise ValueError(
            "Files materialization context participant "
...
def _participant_dir(files_root, participant_hex):
    return pathlib.Path(files_root) / "participants" / participant_hex


def _checkouts_db_path(files_root, participant_hex):
    return _participant_dir(files_root, participant_hex) / "checkouts.db"
...
def _team_metadata_path(files_root, context):
    return _team_dir(files_root, context) / "metadata.json"


def _write_team_metadata(files_root, context):
    path = _team_metadata_path(files_root, context)
...
def materialize_team(files_root, context):
    ...
    _write_team_metadata(files_root, context)


def iter_materialized_teams(files_root, participant_hex):
...
def _registry_git_dir(files_root, context):
    return _team_dir(files_root, context) / "registry" / "git"


def _registry_checkout_dir(files_root, context):
    return _team_dir(files_root, context) / "registry" / "checkout"
...
def _connect_checkouts(files_root, participant_hex):
    db = _checkouts_db_path(files_root, participant_hex)
...
def _peer_sync_row(files_root, participant_hex, context, repo_kind, niche_name, teammate_id):
    context = _validate_context(participant_hex, context)
    conn = _connect_checkouts(files_root, participant_hex)
    row = conn.execute(
        """
...
def get_peer_signal_watermark(files_root, participant_hex, context, teammate_id):
    context = _validate_context(participant_hex, context)
    conn = _connect_checkouts(files_root, participant_hex)
    row = conn.execute(
...
def set_peer_signal_watermark(files_root, participant_hex, context, teammate_id, count):
    context = _validate_context(participant_hex, context)
    conn = _connect_checkouts(files_root, participant_hex)
    conn.execute(
        """
...
def clear_peer_signal_watermark(files_root, participant_hex, context, teammate_id):
    context = _validate_context(participant_hex, context)
    conn = _connect_checkouts(files_root, participant_hex)
    conn.execute(
...
def _ensure_registry(files_root, participant_hex, context):
...
def init_files(files_root, participant_hex):
    """Create the files directory and local checkout registry database."""
    pdir = _participant_dir(files_root, participant_hex)
...
def create_niche(files_root, participant_hex, context, niche_name):
    ...
    registry_git = _registry_git_dir(files_root, context)
    registry_co = _registry_checkout_dir(files_root, context)
...
def list_niches(files_root, participant_hex, context):
    ...
    for f in sorted(registry_co.glob("*.json")):
        ...
        if data:
            data["residency"] = niche_residency(
                files_root, participant_hex, context, data["name"]
            ).value
            niches.append(data)
    return niches


def add_checkout(files_root, participant_hex, context, niche_name, dest_path):
    ...
    existing = get_checkout(files_root, participant_hex, context, niche_name)
...
def remove_checkout(files_root, participant_hex, context, niche_name, checkout_path):
...
def get_checkout(files_root, participant_hex, context, niche_name):
...
def list_checkouts(files_root, participant_hex, context, niche_name):
    """Return list of checkout paths for a niche (at most one element)."""
    checkout = get_checkout(files_root, participant_hex, context, niche_name)
    return [checkout] if checkout is not None else []


def niche_residency(files_root, participant_hex, context, niche_name):
...
def status(files_root, participant_hex, context, niche_name, checkout_path):
...
def log(files_root, participant_hex, context, niche_name, limit=20):
...
def _require_clean_checkout(files_root, participant_hex, context, niche_name):
    ...
    checkout = get_checkout(files_root, participant_hex, context, niche_name)
    if checkout is None:
        residency = niche_residency(files_root, participant_hex, context, niche_name)
    ...
    if not _is_checkout_clean(checkout, git_dir):
        dirty = [e["path"] for e in status(files_root, participant_hex, context, niche_name, checkout)]
...
def niche_conflict_paths(files_root, participant_hex, context, niche_name):
...
def peer_update_status(
    files_root, participant_hex, context, repo_kind, niche_name, teammate_id
):
    ...
    if repo_kind == "registry":
        _ensure_registry(files_root, participant_hex, context)
        git_dir = _registry_git_dir(files_root, context)
        work_tree = _registry_checkout_dir(files_root, context)
    else:
        git_dir = _niche_git_dir(files_root, context, niche_name)
        if not git_dir.exists():
            row = _peer_sync_row(
    ...
    ref_name = _peer_ref_name(teammate_id)
    parked_sha = _resolve_ref(git_dir, ref_name)
    row = _peer_sync_row(files_root, participant_hex, context, repo_kind, niche_name, teammate_id)
    if parked_sha is None:
        return {
            "teammate_id": teammate_id,
            "repo_kind": repo_kind,
