...
def _init(playground_dir):
    init_files(playground_dir, PARTICIPANT)
...
def test_create_niche(playground_dir):
    ...
    assert "SmallSeaCollectiveFiles" not in str(git_dir)
...
def test_same_friendly_name_different_teams_do_not_share_storage(playground_dir):
    ...
    from ssc_files import files as files_module

    teammate_id = "ff" * 16
    files_module._record_peer_fetch(
    ...
    alpha_status = files_module.peer_update_status(
        playground_dir, PARTICIPANT, alpha, "niche", "docs", teammate_id
    )
    beta_status = files_module.peer_update_status(
    ...
    files_module.set_peer_signal_watermark(playground_dir, PARTICIPANT, alpha, teammate_id, 7)
    files_module.set_peer_signal_watermark(playground_dir, PARTICIPANT, beta, teammate_id, 9)
    assert files_module.get_peer_signal_watermark(
...
def test_iter_materialized_teams_skips_invalid_metadata(playground_dir):
    ...
    from ssc_files.files import iter_materialized_teams, materialize_team
...
def test_add_checkout(playground_dir):
    ...
    # Checkout contains no files/git bookkeeping files
...
def test_merge_dirty_tracked_file_raises(playground_dir):
    ...
    # Bob has a fetched ref parked (from _two_files_setup).
...
def test_no_transit_dir_ever_created(playground_dir):
    ...
    for files_root in [pathlib.Path(playground_dir), bob_root]:
        for path in files_root.rglob("transit"):
            if path.is_dir():
...
def test_fetch_without_checkout_pins_ref(playground_dir):
    ...
    from ssc_files.files import _niche_git_dir, _peer_ref_name, _resolve_ref
...
def test_cwd_preserved_across_files_operations(playground_dir):
    ...
    bob2_root = playground / "files-bob2"
    init_files(str(bob2_root), PARTICIPANT_C)
    from ssc_files.files import _niche_git_dir, _init_git_dir
